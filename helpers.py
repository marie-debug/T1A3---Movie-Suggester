...
def create_file(filename, data):
    """
    Takes a filename and and data and creates and saves a file and prints file path
        ...
        with open(filename+".txt", "w", encoding="utf8") as f:
            for movie_info in data:
                if 'name' in movie_info:
                    f.write(
                        f"Title:{movie_info['name']}\n\nSummary:\n\n{movie_info['overview']}\n\nRating:{str(movie_info['vote_average'])}\n================================\n\n")
                else:
                    f.write(
                        f"Title:{movie_info['title']}\n\nSummary:\n\n{movie_info['overview']}\n\nRating:{str(movie_info['vote_average'])}\n================================\n\n")
        print_text_green(f"your file has been successfully saved at {os.getcwd()}")
